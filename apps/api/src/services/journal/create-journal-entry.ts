import { JournalEntriesRepository } from '@/repositories/journal-entries-repository'

export interface CreateJournalEntryServiceRequest {
  userId: string
  date: Date
  content: string
}

export interface CreateJournalEntryServiceResponse {
  entry: {
    id: string
    date: string
    content: string
    createdAt: Date
    updatedAt: Date
  }
}

export class CreateJournalEntryService {
  constructor(private journalEntriesRepository: JournalEntriesRepository) {}

  async execute({ userId, date, content }: CreateJournalEntryServiceRequest): Promise<CreateJournalEntryServiceResponse> {
    const entry = await this.journalEntriesRepository.create({ userId, date, content })
    const dateOnly = entry.date instanceof Date ? entry.date.toISOString().slice(0, 10) : String(entry.date).slice(0, 10)
    return {
      entry: {
        id: entry.id,
        date: dateOnly,
        content: entry.content,
        createdAt: entry.created_at,
        updatedAt: entry.updated_at,
      },
    }
  }
}
